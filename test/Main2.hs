...
data TestEnv =
  ...
  , withConnReadOnly :: forall a. (Database -> IO a) -> IO a
    -- ^ Like 'withConn', but every invocation shares the same database.
  }
...
regressionTests =
    ...
    , TestLabel "ReadOnly"      . testReadOnly
...
withTestEnv tempDbName cb =
    withConn $ \conn ->
        cb TestEnv
            ...
            , withConnReadOnly = withConnPath (T.pack tempDbName)
                                              [SQLOpenReadOnly]
            }
  where
